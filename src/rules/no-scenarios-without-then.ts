...
const name = 'no-scenarios-without-then';

const availableConfigs = {
  /**
   * Whether a `Then` in a Background counts for the scenarios under it.
   *
  ...
  run(feature, _file, configuration) {
    ...
    for (const {scenario, rule: containingRule} of scenariosOf(feature)) {
      ...
      if (!keywords.includes('then')) {
        errors.push({
          message: `${getNodeType(scenario, feature.language)} "${scenario.name}" does not have a Then step`,
          rule: name,
